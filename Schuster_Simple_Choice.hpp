#ifndef SCHUSTER_SIMPLE_CHOICE_HPP
#define SCHUSTER_SIMPLE_CHOICE_HPP
...
namespace Schuster_Simple_Choice{
 ...
 *
 *
 * This heuristic only saved the first 6-cycle that is found (as long as it exists)
 *
 * Restrictions
...
std::function<bool()> main_ch = []{
    ...
    if(forced_vertices.size() > 0){
        ...
        if(six_cycle != nullptr){
            ...
            // get any forced edge in the sc
            for(int i : *six_cycle){
                if(forced_in_current.contains(i)){
                    v = i;
                    break;
                }
            }
...
void remove(int v, int w, bool check_sc){
    ...
    // check if any of the vertices were part of a six cycle, in some cases (like contraction, a six cycle removal should not be checked)
    if(check_sc){
        ...
        for(const auto& [y, c_y] : G[x]){
            ...
            for(const auto& [z, c_z] : G[y]){
                ...
                if(u != y && G[z].contains(u) && !forced_in_current[z].contains(u)){
                    // add the six cycle
                    six_cycle = new std::set<int>({v,w,u,x,y,z});
                    if(six_cycle->size() != 6){
                        delete six_cycle;
                        six_cycle = nullptr;
                        continue;
                    }
                    std::function<bool()> uninsert_six_cycle = []{
                        delete six_cycle;
                        six_cycle = nullptr;
                        return false;
                    };
...
void remove_six_cycle_vertices(int v, int w){
    // check if the six cycle contains both v and w, remove if true
    if(six_cycle != nullptr && six_cycle->contains(v) && six_cycle->contains(w)){
        delete six_cycle;
        six_cycle = nullptr;
    }
}
...
void remove_six_cycle_vertices(int v){
    ...
    if(six_cycle != nullptr && six_cycle->contains(v)){
        delete six_cycle;
        six_cycle = nullptr;
    }
}
