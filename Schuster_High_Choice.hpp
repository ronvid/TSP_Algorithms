        ...
        for(const auto& [y, c_y] : G[x]){
            ...
            for(const auto& [z, c_z] : G[y]){
                ...
                if(u != y && G[z].contains(u) && !forced_in_current[z].contains(u)){
                    ...
                    // check if the new sc has more forced vertices than the old one
                    int sc_forced = 0;
                    for(int i : *sc){
                        if(forced_in_current.contains(i)) sc_forced++;
                    }

                    if(sc_forced <= six_cycle_forced){
                        delete sc;
                        continue;
                    }

                    // for the lambda function
                    std::set<int>* old_sc = six_cycle;
                    int old_sc_forced = six_cycle_forced;
